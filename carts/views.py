...
def add_cart(request, product_id):
    ...
    try:
        cart = Cart.objects.get(cart_id=_cart_id(request)) # get the cart using the cart_id present the session.
    except Cart.DoesNotExist:
        cart = Cart.objects.create(cart_id=_cart_id(request))
    cart.save()


    # is_cart_item_exists = CartItem.objects.all().filter(product=product, cart=cart).exists()
