...
def payments(request):
    ...
    #store transactions details inside Payment method.
    payment = Payment(
        user = request.user,
...
def place_order(request, total=0, quantity=0):
    ...
    #if cart item count is less than or equal to 0 then redirec backto shop
...
def order_complete(request):
    ...
    try:
        ...
        context = {'order':order, 'ordered_products':ordered_products, 'order_number':order.order_number, 'transID':payment.payment_id, 'payment':payment, 'subtotal':subtotal}
